...
export default function Home() {
  const [user, setUser] = useState("prathambhalke");
    ...
    <div className="flex min-h-screen w-full bg-stone-100 p-1.5 sm:p-4 pt-10 sm:pt-12 transition-all dark:bg-slate-900">
      <div className="mx-auto flex w-full max-w-[600px] flex-col gap-8 rounded p-2">
        ...
        <section className="flex flex-col gap-6">
          <SearchAndButton value={user} onChange={(e) => setUser(e.target.value)} onSubmit={handleUserSubmit} />
          {data?.message ? (
            ...
            <main className="flex w-full flex-col gap-6 rounded-lg bg-white dark:bg-slate-800 px-4 py-8 min-h/-[200px]">
              ...
              <section className="flex gap-2">
                {/* image section */}
                <Image
  src={(data?.avatar_url || data?.documentation_url) ? data?.avatar_url ?? data?.documentation_url : ""}
...
/>
                ...
                <section className="flex flex-col justify-between gap-1 transition-all sm:w-full sm:flex-row">
                  <div>
                    {/* name */}
                    <h1>{data?.name}</h1>
                    <Link target="_blank" href={`https://github.com/${data?.login}`} className="text-blue-500 text-sm hover:underline">
                      {data?.login}
                    </Link>
                    {/* user id */}
                  </div>
              ...
              <section className="flex flex-col gap-5">
                <p>{data?.bio ?? <span className="opacity-60">This profile has no bio</span>}</p>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 min-h-[47px]">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <Link target="_blank" href={data?.blog ?? "#"} className="hover:underline opacity-60 max-w-[200px] overflow-hidden text-ellipsis">
                      {data?.blog ?? <span className="opacity-60"> Not Available</span>}
                    </Link>
                  </div>
                  <div className="flex gap-2 items-center">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <p>{data?.company ?? <span className="opacity-60"> Not Available</span>}</p>
                  </div>
                </div>
              </section>
            </main>
